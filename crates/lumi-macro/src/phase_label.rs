...
pub fn derive_phase_label(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let lumi_renderer = get_lumi("renderer");
    let (impl_generics, ty_generics, where_clause) = input.generics.split_for_impl();

    let impl_into = impl_into(&input.data);

    let expanded = quote! {
        impl #impl_generics ::std::convert::Into<#lumi_renderer::PhaseLabel> for #name #ty_generics #where_clause {
            fn into(self) -> #lumi_renderer::PhaseLabel {
                #impl_into
            }
        }
    };
    ...
    match data {
        Data::Enum(data) => {
            let variants = data.variants.iter().enumerate().map(|(i, variant)| {
                ...
                quote! {
                    Self::#ident => #lumi_renderer::PhaseLabel::new::<Self>(#i),
                }
            });
            ...
            quote! {
                match self {
                    #(#variants)*
                    _ => #lumi_renderer::PhaseLabel::new::<Self>(0),
                }
            }
        }
